...
- [ ] **Part 2 — Verified-actuation framework + device health** (S1). First-class "command → verify
...
- [ ] **Part 3 — Identity, roles & capabilities (RBAC)** (S1). Authenticated principals
...
- [ ] **Part 4 — Multi-property control plane** (S2). Generalize the two-house model to N properties
      with per-property isolation and a portfolio view — the estate/family-office shape.
- [ ] **Part 5 — Native life-safety export** (S1). Emit the life-safety subset (leak, fire/CO, freeze)
      as native Home Assistant / Node-RED automation definitions so they survive the Python process.
- [ ] **Part 6 — Operator dashboard** (S2). A real oversight UI for the managed-monitoring tier.
