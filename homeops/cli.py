...
def _deployment_yaml(kind: str) -> str:
    head = ("# Generated by `homeops init`. Lifecycle: validate -> preflight -> serve.\n"
            ...
            "deployment:\n")
    common = ("  houses_config: houses.yaml\n"
...
def init(argv: list[str]) -> int:
    ...
    if _os.path.exists(path):
        ...
        return 1
    # houses_config resolves relative to the deployment file, so seed a local
    # houses.yaml from the packaged example (non-destructive: reuse if present).
    if not _os.path.exists("houses.yaml"):
        from .bootstrap import DEFAULT_CONFIG as _example
        try:
            import shutil as _shutil
            _shutil.copyfile(_example, "houses.yaml")
            print("wrote houses.yaml  (copied example \u2014 edit to match your properties)")
        except OSError as e:
            print(f"cannot seed houses.yaml from {_example!r}: {e}")
            return 1
