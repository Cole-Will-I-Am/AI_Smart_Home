...
def test_advisory_inference_event_cannot_trigger_a_routine_action(bare):
    bare.routines.clock = at(12)
    with pytest.raises(ValueError):
        bare.routines.install(Routine(
            "rt-bad-inference-trigger",
            {"event_type": "inference", "inference_type": "leak_suspected"},
            [{"subsystem": "light", "target": "kitchen", "action": "turn_on"}],
            "colton",
            "house_a",
        ), OWNER)

    bare.bus.publish(Event("inference", "house_a", None, {"inference_type": "leak_suspected"}, bare.engine.tick))
    bare.routines.evaluate_tick()

    assert bare.state.get_state("house_a.light.kitchen") == "off"
    assert not any(r.status == "routine_fired" for r in bare.audit.records)
